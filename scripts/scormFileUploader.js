/**
 * ScormScormFileUploader widget module.
 *
 * @param {H5P.jQuery} $
 * @param {EventDispatcher} EventDispatcher
 */
H5PEditor.ScormFileUploader = (function($, EventDispatcher) {
  var nextIframe;

  /**
   * File Upload API for H5P
   *
   * @class H5PEditor.ScormFileUploader
   * @extends H5P.EventDispatcher
   * @param {Object} field Required for validating the uploaded file
   */
  function ScormFileUploader(field) {
    var self = this;
    ...
    var upload = function(data) {
      if (!nextIframe.isReady()) {
        // Iframe isn't loaded. The caller should try again later.
        return false;
      }
  ...
  ScormFileUploader.prototype = Object.create(EventDispatcher.prototype);
  ScormFileUploader.prototype.constructor = ScormFileUploader;

  /**
   * Iframe for file uploading. Only available for the ScormFileUploader class.
  ...
  function Iframe() {
    ...
    var insertForm = function() {
      ...
      $form = $('<form/>', {
        ...
        action: H5PEditor.getAjaxUrl('scorm')
      });
      ...
      if (response) {
        ...
        if (result !== undefined) {
          ...
          if (result.success === false) {
            uploadComplete.error = (result.message ? result.message : H5PEditor.t('core', 'unknownScormFileUploaderror'));
          }
        }
